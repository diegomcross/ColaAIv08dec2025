...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        """)
        # Ciclo de Vida (COM CORREÇÃO DE DUPLICAÇÃO)
        await db.execute("""
            CREATE TABLE IF NOT EXISTS event_lifecycle (
                ...
                reminder_4h_sent BOOLEAN DEFAULT 0,
...
async def get_recent_masters(weeks=3):
    # FIX: Usando BR_TIMEZONE para evitar erro de naive/aware
...
async def get_last_activity_timestamp(user_id):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        try:
            # Garante que timestamps recuperados tenham fuso se forem strings
            ...
            # Normaliza para comparação (remove tz para comparar cru ou adiciona se ambos precisarem)
            # A estratégia mais segura aqui é converter ambos para BR_TIMEZONE se possível
