...
async def send_onboarding_log(guild, member, status_type, app_data, moderator=None, extra_info=None):
    ...
    if status_type == 'JOIN':
        color = discord.Color.blue(); title = "NOVO MEMBRO SE JUNTOU"
    elif status_type == 'APPROVE':
        color = discord.Color.green(); title = "NOVO MEMBRO ACEITO"
    elif status_type == 'REJECT':
        color = discord.Color.red(); title = "NOVO MEMBRO RECUSADO"
    else:
        color = discord.Color.dark_grey(); title = "NOVO MEMBRO SAIU"
    ...
    desc = (f"**Bungie ID:** `{app_data.get('bungie_id', 'N/A')}`\n"
            f"**Discord:** {member.mention} (`{member.name}`)\n"
            f"**Perfil:** {tags_str}\n")

    if status_type == 'APPROVE' and moderator: desc += f"**Aprovado por:** {moderator.mention}\n"
    elif status_type == 'REJECT' and moderator: desc += f"**Recusado por:** {moderator.mention}\n"
...
class StaffApprovalView(ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: ui.Button):
        ...
        allowed_roles = [config.ROLE_MOD_ID, config.ROLE_FOUNDER_ID, config.ROLE_ADMIN_ID]
        ...
        try:
            embed_dm = discord.Embed(title="🚀 Acesso Aprovado!", description="Bem-vindo ao Clã! Agora você tem acesso total ao servidor.", color=discord.Color.green())
            await self.member.send(embed=embed_dm)
        except: pass

        main_chat = guild.get_channel(config.CHANNEL_MAIN_CHAT)
        if main_chat:
            await main_chat.send(f"👋 **Olhos para cima, Guardiões!**\nUm novo membro foi aprovado: Seja bem-vindo(a), {self.member.mention}! 🚀\nIdentidade: `{self.app_data['bungie_id']}`")
...
class BungieRequestView(ui.View):
    ...
    async def confirm_sent(self, interaction: discord.Interaction, button: ui.Button):
        ...
        embed_wait = discord.Embed(title="🔄 Aguardando Aprovação", description="Obrigado! Notifiquei os moderadores.", color=discord.Color.orange())
        ...
        freq_str = "Normal"
        alert_freq = False
        if config.ROLE_FREQ_SEM_TEMPO in roles_selected: freq_str = "⚠️ Sem Tempo"; alert_freq = True
        elif config.ROLE_FREQ_RARA in roles_selected: freq_str = "⚠️ Raro"; alert_freq = True
        ...
        embed_staff = discord.Embed(title="🛡️ Nova Solicitação Pendente", description=f"Usuário: {self.member.mention}", color=discord.Color.blue())
...
class VoiceOathModal(ui.Modal, title="Termo de Compromisso"):
    ...
    def __init__(self, bot, app_data, member): super().__init__(); self.bot = bot; self.app_data = app_data; self.member = member
    async def on_submit(self, interaction):
        if self.confirmation.value.strip().lower() != "eu concordo em participar das calls": return await interaction.response.send_message("❌ Frase incorreta.", ephemeral=True)
        await interaction.response.defer()
...
class VoiceOathView(ui.View):
    def __init__(self, bot, app_data, member): super().__init__(timeout=None); self.bot = bot; self.app_data = app_data; self.member = member
    @ui.button(label="📝 Assinar Compromisso", style=discord.ButtonStyle.green, emoji="🎙️")
    async def sign_oath(self, interaction, button): await interaction.response.send_modal(VoiceOathModal(self.bot, self.app_data, self.member))
...
class SetupModal(ui.Modal, title="Identificação"):
    ...
    def __init__(self, bot, member): super().__init__(); self.bot = bot; self.member = member
    async def on_submit(self, interaction):
        clean_id = re.sub(r'\s*#\s*', '#', self.bungie_id.value.strip())
        if not re.match(r'^.+#\d+$', clean_id): return await interaction.response.send_message("❌ Formato inválido. Use Nome#1234", ephemeral=True)
...
class StartOnboardingView(ui.View):
    def __init__(self, bot, member): super().__init__(timeout=None); self.bot = bot; self.member = member
    @ui.button(label="🚀 Iniciar Registro", style=discord.ButtonStyle.primary)
    async def start(self, interaction, button): await interaction.response.send_modal(SetupModal(self.bot, self.member))

class WelcomeCog(commands.Cog):
    def __init__(self, bot): self.bot = bot; self.cleanup_channels_loop.start()
    def cog_unload(self): self.cleanup_channels_loop.cancel()
    @commands.Cog.listener()
    async def on_member_remove(self, member):
        pending = await db.get_pending_join(member.id)
        if pending:
            await send_onboarding_log(member.guild, member, 'LEAVE', pending, extra_info="KICKADO / QUITOU")
            await db.remove_pending_join(member.id)
    @commands.Cog.listener()
    async def on_member_join(self, member):
        guild = member.guild
        category = guild.get_channel(config.CATEGORY_WELCOME_ID)
        overwrites = {guild.default_role: discord.PermissionOverwrite(read_messages=False), member: discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True), guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_channels=True)}
        # FIX: Added ROLE_ADMIN_ID to overwrites
        for rid in [config.ROLE_MOD_ID, config.ROLE_FOUNDER_ID, config.ROLE_ADMIN_ID]:
            r = guild.get_role(rid)
        ...
        except Exception as e: print(f"[WELCOME] Erro: {e}")
                ...
                try:
                    ...
                    if (now - last_msg_time).total_seconds() > 86400: # 24h
                        ...
                        for target in channel.overwrites:
                            if isinstance(target, discord.Member) and not target.bot: target_member = target; break
                        if target_member:
