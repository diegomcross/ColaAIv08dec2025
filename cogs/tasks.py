...
if hasattr(config, 'GEMINI_API_KEY') and config.GEMINI_API_KEY:
    try:
        ...
        # Mudamos para 'gemini-pro' que é mais estável e universalmente aceito na API v1beta
        model = genai.GenerativeModel('gemini-pro')
    except Exception as e:
        model = None
        print(f"[AVISO] Erro ao configurar IA: {e}")
else:
    model = None
    print("[AVISO] GEMINI_API_KEY não encontrada no config. O bot usará frases fixas.")

# --- LISTAS DE FALLBACK (Caso a IA falhe ou não tenha chave) ---
...
class TasksCog(commands.Cog):
    ...
    async def daily_morning_loop(self):
        # Delay Aleatório (0 a 2 horas = 0 a 7200 segundos)
    ...
    async def info_board_loop(self):
        ...
        try:
            ...
            # Ignora erros de conexão temporários para não sujar o log
